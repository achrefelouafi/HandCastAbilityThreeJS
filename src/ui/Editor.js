...
export class Editor {
  ...
  _buildTwilight() {
    ...
    /* ---- panel 3 ---- */
    const tip = folder.addFolder('3 · The burning tip (the body)');
    R(tip, c, 'tipLength', 0.2, 12, 0.05, 'apex to the longest tooth (m)');
    R(tip, c, 'tipRadius', 0.02, 3, 0.01, 'body radius (m)');
    R(tip, c, 'tipSwell', 0.05, 1, 0.01, 'needle swells to a body by');
    R(tip, c, 'tipFlare', 0.2, 4, 0.01, 'sharpness (>1 needle)');
    R(tip, c, 'tipSplay', 0, 2, 0.01, 'the back keeps opening');
    R(tip, c, 'tipTeeth', 1, 40, 1, 'teeth around it');
    R(tip, c, 'tipBodyEnd', 0.05, 0.95, 0.01, 'solid body ends at');
    R(tip, c, 'tipToothLength', 0.02, 1, 0.01, 'a short tooth reaches');
    R(tip, c, 'tipStreamers', 0, 1, 0.01, 'streamers (fraction)');
    R(tip, c, 'tipToothWidth', 0.05, 1, 0.01, 'tooth fills its slot');
    R(tip, c, 'tipToothTaper', 0.2, 4, 0.05, 'comes to a point (>1 sharp)');
    R(tip, c, 'tipToothLean', 0, 2, 0.01, 'tooth leans sideways');
    R(tip, c, 'tipFlicker', 0, 0.3, 0.005, 'reach flickers by');
    R(tip, c, 'tipFlickerSpeed', 0, 12, 0.05, 'flicker speed');
    R(tip, c, 'tipCurl', 0, 1.5, 0.01, 'tooth swings off (rad)');
    R(tip, c, 'tipCurlScale', 0.1, 8, 0.05, 'curl scale');
    R(tip, c, 'tipCurlSpeed', 0, 8, 0.05, 'curl speed');
    R(tip, c, 'tipSpread', 0, 1, 0.01, 'tooth leaves the envelope');
    R(tip, c, 'tipRipple', 0, 1, 0.005, 'off a clean teardrop');
    ...
    R(tip, c, 'tipErode', 0, 1.5, 0.01, 'tooth edges eaten into');
    ...
    R(tip, c, 'tipHeatFalloff', 0.05, 4, 0.01, 'heat drops off behind the apex');
    R(tip, c, 'tipRimCool', 0, 1, 0.01, 'the silhouette cools');
    R(tip, c, 'tipRimPower', 0.2, 8, 0.05, 'rim tightness');
    R(tip, c, 'tipToothCool', 0, 1, 0.01, 'a tooth cools to its point');
    R(tip, c, 'tipBands', 1, 8, 1, 'flat tones');
    R(tip, c, 'tipPosterize', 0, 1, 0.01, 'pushed toward the tones');
    R(tip, c, 'tipWobble', 0, 0.5, 0.005, 'tone edges wander');
    R(tip, c, 'tipWobbleScale', 0.1, 8, 0.05, 'wander scale');
    R(tip, c, 'tipWobbleSpeed', 0, 8, 0.05, 'wander speed');
    R(tip, c, 'tipApex', 0, 4, 0.01, 'heat at the point');
    R(tip, c, 'tipApexTight', 0.5, 16, 0.1, 'how tight that is');
    R(tip, c, 'tipCoreScale', 0.05, 1, 0.01, 'the core, x the radius');
    R(tip, c, 'tipCoreLength', 0.05, 1.2, 0.01, 'the core, x the length');
    R(tip, c, 'tipCoreHeat', 0, 1, 0.01, 'the core runs hotter by');
    R(tip, c, 'tipInner', 0, 1.5, 0.01, 'the inside, through the gaps');
    R(tip, c, 'tipIntensity', 0, 6, 0.01, 'intensity');
    R(tip, c, 'tipRolloff', 0.02, 2, 0.01, 'roll-off (keeps the colour)');
    R(tip, c, 'tipOpacity', 0, 1.5, 0.01, 'opacity');
    R(tip, c, 'tipSoftFade', 0.02, 2, 0.01, 'soft fade (m)');
    R(tip, c, 'tipBurstFlare', 0, 4, 0.01, 'body blows open');
    tip.addColor(c, 'colorTipCore').name('the point & the heart');
    tip.addColor(c, 'colorTipHot').name('yellow');
    tip.addColor(c, 'colorTip').name('orange');
    tip.addColor(c, 'colorTipBase').name('the red tips');

    const plume = folder.addFolder('3 · ... and the leaves off its flanks');
    R(plume, c, 'plumeTongues', 1, 40, 1, 'leaves');
    R(plume, c, 'plumeRoot', 0, 6, 0.05, 'first root behind the head (m)');
    R(plume, c, 'plumeRootSpread', 0, 6, 0.05, 'roots strung out over (m)');
    R(plume, c, 'plumeRootRadius', 0, 1.5, 0.01, 'roots off the axis (m)');
    R(plume, c, 'plumeLength', 0.2, 12, 0.05, 'streams back (m)');
    R(plume, c, 'plumeSplay', 0, 4, 0.01, 'off the axis at the point (m)');
    R(plume, c, 'plumeSplayPow', 0.2, 5, 0.05, 'opens late');
    R(plume, c, 'plumeFlatten', 0, 1, 0.01, 'fan depth (0 flat to the view)');
    R(plume, c, 'plumeWidth', 0.01, 1.2, 0.005, 'half-width at the root (m)');
    R(plume, c, 'plumeNoseWidth', 0.02, 1, 0.01, 'pinch at the root');
    R(plume, c, 'plumeTaper', 0.1, 6, 0.05, 'comes to a point (1 straight)');
    ...
    R(plume, c, 'plumeEat', 0, 1.5, 0.01, 'edge eaten into');
    R(plume, c, 'plumeEatScale', 0.1, 10, 0.05, 'eat scale');
    R(plume, c, 'plumeEatSpeed', 0, 8, 0.05, 'eat speed');
    R(plume, c, 'plumeHeat', 0.05, 5, 0.05, 'heat drops off to the point');
    R(plume, c, 'plumeEdgeCool', 0, 1, 0.01, 'the edge cools');
    R(plume, c, 'plumeEdgePower', 0.2, 8, 0.05, 'edge tightness');
    R(plume, c, 'plumeCore', 1, 20, 0.5, 'thread up the middle: tightness');
    R(plume, c, 'plumeCoreHeat', 0, 1, 0.01, 'thread up the middle: heat');
    R(plume, c, 'plumeBands', 1, 8, 1, 'flat tones');
    R(plume, c, 'plumePosterize', 0, 1, 0.01, 'pushed toward the tones');
    R(plume, c, 'plumeWobble', 0, 0.5, 0.005, 'tone edges wander');
    ...
    R(plume, c, 'plumeRolloff', 0.02, 2, 0.01, 'roll-off (keeps the orange)');
    ...
    plume.addColor(c, 'colorPlumeHot').name('yellow');
    plume.addColor(c, 'colorPlume').name('orange');
    plume.addColor(c, 'colorPlumeTip').name('the red points');
