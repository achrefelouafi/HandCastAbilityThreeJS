...
const WARP_VERTEX = /* glsl */ `
  ...
  void main() {
    ...
    // Which way forward is on screen.
    vec2 dv = (viewMatrix * vec4(uDir, 0.0)).xy;
    vDirView = length(dv) > 1e-4 ? normalize(dv) : vec2(0.0, 1.0);

    // Camera-facing: the quad's own basis is discarded and its corners are
...
const WARP_FRAGMENT = /* glsl */ `
  ...
  uniform float uTime;
  uniform float uSeed;
  uniform float uLens;         // the bulb that displaces along the heading
  uniform float uLensPower;
  uniform float uSwirl;        // the vortex about the head
  uniform float uArms;         // spiral arms in it
  uniform float uSpiral;       // how tightly they wind
  uniform float uSpin;         // turns/second
  uniform float uChurn;
  uniform float uScale;
  uniform float uSpeed;
  uniform float uWave;         // ring packets shed off the head
  ...
  void main() {
    ...
    /* --- the swirl: the frame turned about the head, in spiral arms --- */
    vec2 radial = d > 1e-4 ? c / d : vec2(0.0, 1.0);
    vec2 tangential = vec2(-radial.y, radial.x);
    float ang = atan(c.y, c.x);
    float arm = 0.5 + 0.5 * sin(ang * uArms + d * uSpiral - uTime * uSpin * TAU + uSeed);
    float swirl = uSwirl * sin(clamp(d / 0.85, 0.0, 1.0) * PI) * (0.35 + 0.65 * arm);

    /* --- boiling inside the lens --- */
    ...
    float mask = clamp(lens + abs(swirl) + waveAmp + burstAmp, 0.0, 1.0) * edge * uFade;
 ...
 *   `{ dir, size, seed, burst, burstFront, fade }`
 */
export function createVoidWarpMaterial() {
  const material = new ShaderMaterial({
    name: 'VoidWarp',
    transparent: true,
    depthWrite: false,
  ...
  material.userData.sync = (state) => {
    ...
    u.uArms.value = Math.max(1, Math.round(c.warpArms));
    u.uSpiral.value = c.warpSpiral;
    u.uSpin.value = c.warpSpin * g.animationSpeed;
